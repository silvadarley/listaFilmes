import React, { useEffect, useState } from 'react';
import { Link, useParams } from "react-router-dom";
import {MovieService} from "../../api/MovieService";
import axios from 'axios';

const MovieDetail = () => {
  const {id} = useParams();
/*
  Aqui vou receber uma variavel de estado para que seja dinamico de acordo com o filme selecionado
  Fazendo isso eu não preciso de colocar o movie no parametro inicial da função.]
  Caso haja duvida consultar documentação em hooks por ******useParams*********
*/
  const [movie, staeMovie] = useState({});
  
  async function getMovie(){
    const {data} = await MovieService.getMoviesDetails(id);
    staeMovie(data);
  }

  /* Quando eu for chamar um método devo fazer como está a seguir
        ******[Colchete vazio ]***** para que ele so chame o métoo uma vez*/
  useEffect(() =>{
    getMovie();
  }, []);

  return (
    <section className="MovieDetail">
      <div className="MovieDetail__container">

        <div className="MovieDetail__col">

          <h1 className='MovieDetail__title'>
            {movie.titele}
          </h1>

          <div className="MovieDetail__image">
            <img
              src={`https://image.tmdb.org/t/p/w1280${movie.backdrop_path}`}
              alt={movie.title}
            />
          </div>

        </div>


        <div className="MovieDetail__col">

          <div className="MovieDetaildetails">

            <div className="MovieDetail__detail">
              <span>Budget:</span> {movie.budget}
            </div>

            <div className="MovieDetail_detail">
              <span>Original language:</span> {movie.original_language}
            </div>

            <div className="MovieDetail__detail">
              <span>Popularity:</span> {movie.popularity}
            </div>

            <div className="MovieDetail__detail">
              <span>Overview:</span> {movie.overview}
            </div>
          </div>
          <Link to={"/"} className='MovieDetail__button'>
            Voltar
          </Link>
        </div>
      </div>
    </section>
  )
}

export default MovieDetail
